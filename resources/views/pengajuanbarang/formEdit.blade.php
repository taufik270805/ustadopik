...
<script>
    $('#modalEdit').on('show.bs.modal', function(e){
        ...
        const namaP = btn.data('nama_pengaju');
        const namaB = btn.data('nama_barang');
        const tgl = btn.data('tanggal_pengajuan');
        const qty = btn.data('qty');
        const id = btn.data('id');
        const modal = $(this);
        modal.find('#nama_pengaju').val(namaP);
        modal.find('#nama_barang').val(namaB);
        modal.find('#tanggal_pengajuan').val(tgl);
        modal.find('#qty').val(qty);
        $('#formEdit').attr('action', '{{ url("/barang") }}/'+id);
